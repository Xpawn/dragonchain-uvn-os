...
EOL
    ...
    # Mount points
    if [[ $(grep 'mount -o remount,exec /tmp' "/etc/apt/apt.conf.d/999-local") ]]; then
        cat > /etc/apt/apt.conf.d/999-local <<- EOT
DPkg::Pre-Invoke{"mount -o remount,exec /tmp";};
DPkg::Post-Invoke {"mount -o remount,noexec /tmp";};
EOT
        sed -i '/^\/dev\/sr0/d' /etc/fstab;
        sed -i 's/errors=remount-ro/rw,noatime,journal_checksum,errors=remount-ro/g'
    ...
    # Snap
    [[ $(snap find core | grep '^core') ]] || snap install core;
    [[ $(snap find microk8s | grep 'microk8s') ]]  || snap install microk8s --channel=1.18/edge --classic;
...
{
    if [[ ! -z "${MY_EMAIL}" ]]; then
        certbot register --agree-tos --no-eff-email -m ${MY_EMAIL};
    fi
...
function config_microk8s()
{
    ...
    runuser -l  ${MY_USER} -c 'microk8s.status --wait-ready && microk8s.enable storage && microk8s.enable dns && sleep 3;';
    runuser -l  ${MY_USER} -c 'microk8s.enable helm3 && sleep 1 && microk8s.enable registry && sleep 2;';
    runuser -l  ${MY_USER} -c '/snap/bin/helm repo add dragonchain https://dragonchain-charts.s3.amazonaws.com && /snap/bin/helm repo update;';
    
    echo '{"kind":"Namespace","apiVersion":"v1","metadata":{"name":"dragonchain","labels":{"name":"dragonchain"}}}' | /snap/bin/kubectl create -f -;
...
function sayonara()
{
    echo -e "\nConfig done!\n\nThis script will be deleted after reboot.\n";
    read -n 1 -spt 5 "Press [ENTER] or wait five seconds for reboot..." r;
    echo
